---
title: "Multiclass - Classification on Iris dataset"
output: html_document
---
Multiclass  classication on iris data set to classify flowers according to different species.
...
library(rattle)           #decision tree model
```
...
iris <- select(iris,-c(Id))   #removing the Id column
head(iris)                    #shows first 6 rows in dataframe
```
```{r }
```
...
```{r split data}
valid_matrix <- createDataPartition(iris$Species,p=0.80,list=FALSE)

test_set <- iris[-valid_matrix,]   #20% for the test
train_set <- iris[valid_matrix,]   #80% for the train
```
...
colnames(train_set)       #column names
sapply(train_set,class)   #class of each column
levels(train_set$Species) #levels/catagories in Species
table(train_set$Species)  #no. of entries in each label
```
##### Feature Plotings :
...
```{r}
```
Bar plot to check the proportion in each species .There are 40 entries for each species.
```{r}
featurePlot(x=x_set,
            y=y_set, 
            plot = "ellipse",
...
#Naive Bayes
set.seed(7)
fit.nb <- train(Species~.,data=train_set,method="nb",trControl= control)

# Classification and Regression Trees
#rpart : non recursive partitioning./decision tree
set.seed(7)
fit.cart <- train(Species~., data=train_set, method="rpart",trControl=control)
fancyRpartPlot(fit.cart$finalModel)

#multinom : Logistic Regression
set.seed(7)
fit.multinom <- train(Species~.,data=train_set,method="multinom")

# k-Nearest Neighbors
set.seed(7)
fit.knn <- train(Species~., data=train_set, method="knn",trControl=control)

# Support Vector Machines
set.seed(7)
fit.svm <- train(Species~., data=train_set, method="svmRadial",trControl=control)

#Random Forest
set.seed(7)
fit.rf <- train(Species~., data=train_set, method="rf",trControl= control)

# Linear Discriminant Analysis
set.seed(7)
fit.lda <- train(Species~., data=train_set, method="lda",trControl=control)
```
...
lda_predict <- predict(fit.lda,test_set)
